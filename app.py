...
for sym, g in df.groupby("symbol"):
    df.loc[g.index, f"vol_sma{vol_sma_period}"] = g["volume"].rolling(vol_sma_period).mean()
    for p in sma_periods:
        df.loc[g.index, f"sma{p}"] = g["close"].rolling(p).mean()
    for p in ema_periods:
        df.loc[g.index, f"ema{p}"] = g["close"].ewm(span=p, adjust=False).mean()
    if enable_rsi:
        df.loc[g.index, f"rsi_{rsi_period}"] = ta.rsi(g["close"], length=rsi_period)
    if enable_adx:
        adx = ta.adx(g["high"], g["low"], g["close"], length=adx_period)
        df.loc[g.index, adx.columns] = adx.values
...
f = latest[latest["close"] >= min_price]
if vol_surge and f.get(f"vol_sma{vol_sma_period}") is not None:
    f = f[f["volume"] > vol_multiplier * f[f"vol_sma{vol_sma_period}"]]
...
st.dataframe(f[cols].sort_values("symbol"), use_container_width=True)
...
if not f.empty:
    ...
    if selected_symbol:
        ...
        fig.update_layout(title=f"{selected_symbol} — Price & MAs", height=600, template="plotly_dark")
